...
// Comprehensive function to get Font Awesome icon class for each food
const getFoodIcon = (foodName) => {
  ...
  // If still no match found, return a general food icon
  return '🍽️';
};
  ...
  return (
    <div className="max-w-6xl mx-auto bg-white rounded-lg shadow-lg overflow-hidden mt-6">
      <div className="bg-red-600 text-white p-4">
        <h2 className="text-3xl font-bold">My Food Guide <span className="emoji-3d">🍽️</span></h2>
        ...
        {member.menu.map((item, index) => (
          <div key={index} className="mb-8">
            <h3 className="text-2xl font-semibold mb-4 flex items-center">
              <span className="emoji-3d mr-2">{getFoodIcon(item.group)}</span>
              {item.group}
            ...
            <div className="food-table">
              {item.foods.map((food, foodIndex) => {
                ...
                return (
                  <div key={foodIndex} className="food-cell">
                    <span className="emoji-3d mb-2">{getFoodIcon(foodName)}</span>
